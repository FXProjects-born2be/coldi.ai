...
import { REQUEST_CALL_TST_FORM_ENABLED } from '@/shared/lib/system/request-forms';
...
export const FirstStepToCall = ({
  ...
  const { Field, Subscribe, handleSubmit, store } = useForm({
    ...
    onSubmit: (data) => {
      if (!REQUEST_CALL_TST_FORM_ENABLED) {
        return;
      }
  ...
  return (
    <section className={st.container}>
      ...
      >
        <TemporarilyDisabledForm disabled={!REQUEST_CALL_TST_FORM_ENABLED}>
          ...
          <Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
            {([canSubmit, isSubmitting]) => (
              <Button
                disabled={!REQUEST_CALL_TST_FORM_ENABLED || !canSubmit || isSubmitting}
                type="submit"
                fullWidth
              >
